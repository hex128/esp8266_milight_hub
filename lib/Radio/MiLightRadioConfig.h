...
public:
  ...
  // We can set this to two possible values. It only affects the nRF24 radio. The
  ...
  //      and the data must be bit-shifted every time it's received.
  //
    ...
    , packetLength(packetLength)
  {
    ...
    if constexpr (SYNCWORD_LENGTH == 5) {
      syncwordBytes[--ix] = reverseBits((syncword0 << 4 & 0xF0) | (preamble & 0x0F));
      ...
      syncwordBytes[--ix] = reverseBits((syncword3 >> 12 & 0x0F) | (trailer << 4 & 0xF0));
    } else {
